...
(function () {
   ...
   * тем же _webp, каким документ строит фигуру. Область видимости — МОДУЛЬ.
   *
   * ЗАПАСНОГО ПУТИ НА `file` БОЛЬШЕ НЕТ, и это не упрощение записи: `file` есть имя
   * АВТОРСКОГО носителя, а не адрес в мире. Пока оригиналы уезжали в мир, разница не
   * была видна; 2026-08-19 каталог страницы стал проекцией источника (−106.7 МБ), и
   * `file` перестал разрешаться. `light` теперь ТОТАЛЕН (materialise ставит его всегда),
   * поэтому одно поле — один смысл, и запасной путь нечем оправдать. */
  const imgSrc = (s) => `img/${(s && s.light) || ''}`;
